...
def parse_arguments():
    ...
    parser.add_argument('--model_interval', type=int, default=15, metavar='N', help='how many batches to wait before logging training status')
...
def compute_original_3_loss(pts_pred, gt_patch_pts, alpha):
    ...
    # print('min_dist: %f max_di4st: %f' % (alpha * torch.mean(min_dist).item(), (1 - alpha) * torch.mean(max_dist).item()))
    return dist * 100
